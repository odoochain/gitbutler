...
use crate::{
    branch::segment_disconnect::determine_parent_selector,
    graph_manipulation::{
        already_connected_parent_for_step, connect_parent_step,
        disconnect_selector_from_all_parents,
    },
};
...
pub enum BranchIntegrationStrategy {
    ...
    Merge,
    /// Rebuild the branch with upstream commits only.
    ResetToRemote,
}
...
pub(super) fn initial_integration_steps(
    ...
    let upstream_steps = upstream_commits
        .iter()
        .rev()
        .copied()
        .map(|commit_id| InteractiveIntegrationStep::Pick { commit_id });
    ...
    match strategy {
        BranchIntegrationStrategy::PullRebase => {
            upstream_steps.chain(local_steps).collect::<Vec<_>>()
        }
        BranchIntegrationStrategy::Merge => {
            let mut steps = local_steps.collect::<Vec<_>>();
        ...
        BranchIntegrationStrategy::ResetToRemote => upstream_steps.collect::<Vec<_>>(),
    }
}
...
///
/// `merge_base` is the commit id that should terminate the rebuilt chain at its
/// bottom.
///
/// `steps` is the prepared execution plan to insert between `ref_name` and
/// `merge_base`.
///
