...
fn initial_integration_for_branch_with_strategy(
    ...
    let graph = if let Some(target_ref_name) = target_ref_name {
        let head = repo.head()?;
...
fn initial_steps_strategy_variants_are_distinct_presets() -> Result<()> {
    ...
    for (name, strategy) in [
        ...
        ("reset-to-remote", BranchIntegrationStrategy::ResetToRemote),
    ] {
...
#[test]
fn initial_steps_reset_to_remote_strategy_picks_only_upstream_commits() -> Result<()> {
    let mut repo = read_only_in_memory_scenario_named(
        "journey03",
        "01-rewritten-local-commit-is-paired-with-remote",
    )?;
    ...
    let initial = initial_integration_for_branch_with_strategy(
        ...
        &repo,
        Some(r("refs/remotes/origin/main")),
        BranchIntegrationStrategy::ResetToRemote,
    )?;
    ...
    assert_eq!(
        ...
        "reset-to-remote should keep only upstream commits in parent-to-child order",
    );
